...
function setupGizmos() {
  ...
  if (gizmoMgr.gizmos.positionGizmo) {
    gizmoMgr.gizmos.positionGizmo.onDragEndObservable.add(() => marcarSucio());
  }
  if (gizmoMgr.gizmos.rotationGizmo) {
    gizmoMgr.gizmos.rotationGizmo.onDragEndObservable.add(() => marcarSucio());
  }
...
async function restaurarProgreso() {
  ...
  if (progreso) {
    ...
    if (buildPhase >= 1 || matematicasValidadas) unlockPhase1();
    if (buildPhase >= 2 || concretoUnlocked) unlockPhase2();
    if (buildPhase >= 4) unlockAll();
    updateOpeningCounters();
  }
